...
public class CloudCacheService implements CacheService, AdminCacheService {
    ...
    public List<Serializable> getBatch(List keys) {
        ...
        if (firstLevelCache != null) {
            ...
            if (canUsePrimary()) {
                ...
                nullKeyresult = (List<Serializable>) secondary.getBatch(nullKeys);
            }
            else {
    ...
    protected void proxyList(List<Serializable> data) {
        if (proxyFactory != null && data != null) {
            for (int i = 0; i < data.size(); i++) {
                Serializable proxy = (Serializable) proxyFactory.createProxy(data.get(i));
                data.set(i, proxy);
            }
        }
    }
    ...
    private boolean canUsePrimary() {
        boolean active = primary != null && isPrimaryOn && primary.isConnected();
        if (!active) {
            log.info("Primary cache is not active.Isconnected=" + primary.isConnected());
        }
        return active;
    }
    
    private boolean canUseSecondary() {
        boolean active = secondary != null && isSecondaryOn && secondary.isConnected();
        return active;
    }
    ...
    public String toString() {
        ...
        builder.append("CloudCacheService [primary=").append(primary).append(
                ", secondary=").append(secondary).append(", firstLevelCache=").append(
                firstLevelCache).append(", l1Hits=").append(l1Hits).append(", l1Missess=").append(
                l1Missess).append(", isPrimaryOn=").append(isPrimaryOn).append(
                ", isSecondaryOn=").append(isSecondaryOn).append("]");
