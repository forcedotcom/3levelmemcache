...
public class MemcachedCacheService implements CacheService, AdminCacheService {
    ...
    public Serializable get(Object key) {
        ...
        try {
            if (key != null) {
                ...
                try {
                    rt = (Serializable) f.get(operationTimeOutMsec, TimeUnit.MILLISECONDS);
                }
                catch (TimeoutException e) {
                    log.warn("Timeout on get key='" + key + "' , timeout="
                            + operationTimeOutMsec + " mseconds. Returning " + rt);
                    f.cancel(false);
                    
                    if (!exceptionsSilentMode) {
                        throw new RuntimeException("Timeout on get key:" + key, e);
                    }
                }
            }
            return rt;
            ...
            log.warn("For information only, there is exception in memcached get method, please ignore it,"
                    + " unable to retrieve key:" + key + ". Ex:" + alle.getMessage());
    ...
    public boolean add(Object key, Serializable obj) {
        try {
            if (key == null) {
                ...
                OperationFuture<Boolean> f = (OperationFuture<Boolean>) client.add(_key,
                        expireTime, obj);
    ...
    public List<Serializable> getBatch(List keys) {
        ...
        BulkFuture<Map<String, Object>> future = client.asyncGetBulk(prefixedKeys);
        ...
        try {
            ...
            log.warn("Timeout on get bulkvalues keys='" + StringUtils.join(keys, ",")
                    + "' Returning null.");
        }
        catch (Exception e) {
            log.error("Exception on getBatch keys='" + StringUtils.join(keys, ",")
                    + "' Returning null.");
        }
    ...
    public void remove(Object key) {
        OperationFuture<Boolean> f = (OperationFuture<Boolean>) client.delete(getKey(key));
        try {
    ...
    private void doFullAppend(Object key, String obj, int tries) {
        if (tries <= 0) {
            log.error("Try[" + tries + "]Can't append this:" + key + " , by this value:"
                    + obj.toString());
            return;
        }
        ...
        String _key = getKey(key);
        try {
            OperationFuture<Boolean> addFuture = (OperationFuture<Boolean>) client.add(
                    _key, expireTime, obj, nonCompressTranscoder);
            if (!addFuture.get()) {
                OperationFuture<Boolean> appendFuture = (OperationFuture<Boolean>) client.append(
                        0, _key, "," + obj, nonCompressTranscoder);
            ...
            // Log only message
            log.warn("Try[" + tries + "]Can't append this:" + key + " , by this value:"
                    + obj.toString() + " try one more time. Ex:" + e.getMessage());
        ...
        try {
            ...
            if (expiration < 0) {
                throw new IllegalArgumentException(
                        "Expiration time cannot be minus. Exptime=" + expiration);
            }
    ...
    private void putWithRetries(Object key, int expiration, Serializable obj, int tries) {
        
        try {
            OperationFuture<Boolean> rv = putWithSlowDownLogic(key, expiration, obj);
            ...
            if (tries <= 0) {
                log.error("Try[" + tries + "] Error while putting object key=" + key
                        + ", obj=" + obj);
                throw new RuntimeException("Cannot put after several attempts.", e);
            }
            else {
                log.warn("Try[" + tries + "]");
                try {
    ...
    private OperationFuture<Boolean> putWithSlowDownLogic(Object key, int expiration,
            Serializable obj) {
        OperationFuture<Boolean> rv = null;
        while (rv == null) {
            try {
                rv = (OperationFuture<Boolean>) client.set(getKey(key), expiration, obj);
            }
    ...
    public void setTimeOutBulkOperation(String timeOutBulkOperation) {
        this.operationTimeOutBulkMsec = NumberUtils.toLong(timeOutBulkOperation,
                DEFAULT_TIMEOUT_BULK_OPERATION);
